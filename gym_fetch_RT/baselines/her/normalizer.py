...
from gym_fetch_RT.baselines.her.util import reshape_for_broadcasting
...
class Normalizer:
    def __init__(self, size, eps=1e-2, default_clip_range=np.inf, sess=None):
        ...
        print('self.local_sum', self.local_sum)
        print('self.local_sumsq', self.local_sumsq)
        print('self.local_count', self.local_count)
        self.sum_tf = tf.get_variable(
            initializer=tf.zeros_initializer(), shape=self.local_sum.shape, name='sum',
            trainable=False, dtype=tf.float32)
        print('self.sum_tf', self.sum_tf)
