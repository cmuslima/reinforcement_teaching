...
from gym_fetch_RT.baselines.her.util import (
    import_function, store_args, flatten_grads, transitions_in_episode_batch, convert_episode_to_batch_major)
from gym_fetch_RT.baselines.her.normalizer import Normalizer
from gym_fetch_RT.baselines.her.replay_buffer import ReplayBuffer
from gym_fetch_RT.baselines.common.mpi_adam import MpiAdam
from gym_fetch_RT.baselines.common import tf_util
...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
        ...
        with tf.variable_scope(self.scope):
            #print('self.stage_shapes.keys()', self.stage_shapes.keys())
    ...
    def close(self):
        ...
        print('closed session in DDPG')
    def train(self, current_batch, n_batches, stage=True):
        ...
        #tvars_vals = [tvars_vals[0]]
        weight_list = []
        #nums = 0
        final_weight_list2 = None
        if current_batch == n_batches-1:
            for var, val in zip(tvars, tvars_vals):
                array_size = np.shape(val)
               
                weight_list.append(val.tolist())
            ...
            for i in range(32):
                ...
                try:
                    ...
                    weights = [item for sublist in weight_list[i] for item in sublist]

                    final_weight_list.append(weights)
                except:
                    final_weight_list.append(weights)
            final_weight_list2 = [item for sublist in final_weight_list for item in sublist]

        return critic_loss, actor_loss, final_weight_list2
    ...
    def _create_network(self, reuse=False):
        ...
        self.sess = tf_util.get_session(True, config = config)
        #self.sess = tf.reset_default_graph() 
        ...
        with tf.variable_scope('g_stats') as vs:
            ...
            #print('self.g_stats',self.g_stats)
        # mini-batch sampling.
