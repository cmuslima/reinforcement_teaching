...
class RobotEnv(gym.GoalEnv):
    def __init__(self, model_path, initial_qpos, n_actions, n_substeps):
        ...
        self.seed()
        #if debug:
        #print('self.seed is only done once')
    ...
    def reset(self):
        ...
        obs = self._get_obs()
        #the value 1 below used to be a 2
        while np.linalg.norm(obs['achieved_goal'] - obs['desired_goal']) <= 1 * self.distance_threshold:
            #print(obs['achieved_goal'])
            #print(obs['desired_goal'])
            #print(f'distance threshold = {self.distance_threshold}')
            #print(np.linalg.norm(obs['achieved_goal'] - obs['desired_goal']))
            #print(f'2 * self.distance_threshold = {2 * self.distance_threshold}')
            self.goal = self._sample_goal().copy()
            obs = self._get_obs()
