...
def plot_data(data,scores,args):
    ...
    for idx, f in enumerate(list(data.keys())):
        ...
        rewards = ['diversity', 'Time-to-threshold', 'LP', 'Huang(2019) reward', 'Fan(2018) reward', 'Matiisen(2017) reward', 'Ruiz(2019) reward', 'Sparse Ruiz(2019) reward']
...
def get_files_single_baseline(args, scores, teacher_returns, teacher_actions, learning_rates, buffer_sizes, batch_sizes, SR, reward_function):
    ...
    return files
def get_transfer_files(args, scores, teacher_returns, teacher_actions, student_lr, learning_rates, buffer_sizes, batch_sizes, SR, reward_function):
    ...
    for env in envs:
        for SR in state_reps:
            for reward_function in rf:
                ...
                try:
                    baseline_files = get_files_single_baseline(args, scores, teacher_returns, teacher_actions, learning_rates, buffer_sizes, batch_sizes, SR, reward_function)
        ...
        for SR in state_reps:
            for reward_function in rf:
                ...
                if args.student_lr_transfer or args.student_transfer:
                    ...
                    transfer_files = get_transfer_files(args, scores, teacher_returns, teacher_actions, student_lr, learning_rates, buffer_sizes, batch_sizes, SR, reward_function)
...
def t_testing(args, baseline1, baseline2):
    teacher = False
    from scipy import stats
