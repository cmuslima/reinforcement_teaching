...
def get_rootdir(args, SR):
    ...
    if args.teacher_agent == 'DQN':
        ...
        #     rootdir = f'./RT/{args.exp_type}/{args.env}/random_student_seeds/{args.saving_method}/single_student/{args.student_type}/{SR}/hidden_size_i_{args.hidden_size_input}_hidden_size_o_{args.hidden_size_output}_teacher_network_h_{args.teacher_network_hidden_size}'    
        ...
        if args.multi_students:
            if args.multi_controller:
                if args.two_buffer:
                    ...
                    rootdir = f'./RT/{args.env}_{args.num_training_episodes}/{args.saving_method}/multi_students/multi_controller/two_buffer'
                else:
                    print("Running experiment with a combined buffer + multiple controllers + multi students")
                    rootdir = f'./RT/{args.env}_{args.num_training_episodes}/{args.saving_method}/multi_students/multi_controller/combined_buffer'
            else:
                print("Running experiment with a combined buffer + single controllers + multi students")
                rootdir = f'./RT/{args.env}_{args.num_training_episodes}/{args.saving_method}/multi_students/single_controller/combined_buffer'
    elif args.teacher_agent == "Random":
        rootdir = f'./RT/{args.env}/random_curriculum'

    elif args.teacher_agent == "No_Teacher":
        rootdir =  f'./RT/{args.env}/target_task_only'
    return rootdir
...
def import_modules(args):
    ...
    if args.env == 'four_rooms' and args.tabular == False:
        ...
        #     return student_train, evaluate_task
        # except:
        from rl_starter_files_master.student_training import student_train
        from rl_starter_files_master.evaluate import evaluate_task

        return student_train, evaluate_task
def import_training_modules(args):
    if args.teacher_agent == 'DQN':
        from training_loop import teaching_training
    else:
        from training_loop_bandit import teaching_training
    return teaching_training
